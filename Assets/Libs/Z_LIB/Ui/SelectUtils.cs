...
public static class SelectUtils {
   ...
   public static MultiSelectClass<T> MakeMultiSelectClass<T>(IEnumerable<T> data, System.Action<ToggleSelectObject<T>> on_toggle = null) {
      MultiSelectClass<T> res = new MultiSelectClass<T>();
      foreach (var t in data) {
         res.elements.Add(new ToggleSelectObject<T> { val = t, on_toggle = on_toggle });
      }
      return res;
   }

   public static SelectClass<T> MakeSelectClass<T>(IList<T> data, System.Action extra_callback = null, bool can_toggle_off = false, System.Action toggle_off = null, bool can_click_selected = false, int default_selected=0) {
   ...
   public class ToggleSelectObject<T> : SelectableObjectMarker {
      ...
      public System.Func<T, bool> can_toggle;

      public System.Action select {
         get {
            if (can_toggle != null && !can_toggle(val))
               return null;
            return () => {
               selected = !selected;
