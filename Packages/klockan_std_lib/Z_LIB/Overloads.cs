...
public static class Overloads {
   ...
   */
   ...
   public static T GetRing<T>(this IList<T> l, int i) {
      i %= l.Count;
      if (i < 0) i += l.Count;
      return l[i];
   }
   ...
   public static T FirstOrDefault<T>(this IEnumerable<T> e, T def=default) {
   ...
   public static Vector2Int Sum(this IEnumerable<Vector2Int> e) {
   ...
   public static void Filter<T>(this List<T> a, System.Predicate<T> p) {
      int n = 0;
      for (int i = 0; i < a.Count; ++i) if (p(a[i])) a[n++] = a[i];
      a.RemoveRange(n, a.Count - n);
   }
